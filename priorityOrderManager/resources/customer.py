...
def panel():
    if session.get("customer_type") == "Admin":
        ...
        customers = list(mongo.db.customers.find({"CustomerID": customer_id}))
...
def add_customer():
    ...
    if not customer_name:
        log_event("customer_add_error", "Müşteri adı eksik.", None)
    ...
    try:
        ...
        log_event("customer_added", f"Müşteri eklendi: {customer_id} - {customer_name}", "Admin", customer_id)
...
def delete_customer():
    ...
    try:
        ...
        if result.deleted_count == 1:
            log_event("customer_deleted", f"Müşteri silindi: {customer_id}", "Admin", customer_id)
...
def customer_orders(customer_id):
    ...
    for order in orders:
        ...
        if product:
            ...
            order['ProductPrice'] = product.get("Price", "Bilinmiyor")
        else:
            order['ProductName'] = "Ürün Bulunamadı"
            order['ProductPrice'] = "Bilinmiyor"
...
def update_budget():
    ...
    try:
        result = mongo.db.customers.update_one({"CustomerID": customer_id}, {"$set": {"Budget": new_budget}})
        if result.matched_count == 1:
            log_event("customer_budget_updated", f"Müşteri {customer_id} bütçesi güncellendi: {new_budget}", "Admin", customer_id)
